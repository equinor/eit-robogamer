...
def get_board_corners(ids, corners): # ids.flatten(), corners.tolist()
    ...
    if result:
        ...
        board_corners = np.array([upleft, downleft, upright, downright])
...
def get_cordinates(ids, corners, objpts, imgpts):
    corners_id = [66, 67, 68, 69]
    try:
        df = []
        if ids.any():
            try:        
                for x in [ids] and x not in corners_id:
                    c = get_corners_by_id(x, ids, corners)
                    px, py = c[:, 0].mean(), c[:, 1].mean()
                    
                    idx = imgpts.index([px, py])
                    coord = objpts[idx]
                    delta_y = c[1, 1] - c[0, 1]
                    delta_x = c[1, 0] - c[0, 0]
...
if __name__ == "__main__":
    ...
    while frame_captured:
        ...
        board_corners = get_board_corners(ids, corners)
        if board_corners:
            tracking_corners_captured = True
        else:
            tracking_corners_captured = False
            print("Please adjust camera position")
        
        while tracking_corners_captured:

            objp_arc, objpts = get_object_points(width)
            imgpts = get_image_points(board_corners, gray, objp_arc, mtx, dist, objpts)
            result = get_cordinates(ids, corners, objpts, imgpts)
